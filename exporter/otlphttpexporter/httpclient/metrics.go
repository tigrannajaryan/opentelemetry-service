...
var mSpansDropped = stats.Int64("sapm/spans_dropped", "Number of items dropped", stats.UnitDimensionless)
var mBatchesExported = stats.Int64("sapm/batches_exported", "Number of batches successfully exporter", stats.UnitDimensionless)
var mSpansExported = stats.Int64("sapm/spans_exported", "Number of items successfully exporter", stats.UnitDimensionless)
...
func recordDrops(ctx context.Context, sr *sendRequest) {
	stats.Record(ctx, mBatchesDropped.M(sr.batches), mSpansDropped.M(sr.items))
}

func recordSuccess(ctx context.Context, sr *sendRequest) {
	stats.Record(ctx, mBatchesExported.M(sr.batches), mSpansExported.M(sr.items))
}
