...
import (
	...
	"github.com/open-telemetry/opentelemetry-collector/receiver/otlpreceiver/otlpmetrics"
	"github.com/open-telemetry/opentelemetry-collector/receiver/otlpreceiver/otlptrace"
)
...
type Receiver struct {
	...
	traceReceiverOpts   []otlptrace.Option
	metricsReceiverOpts []otlpmetrics.Option

	traceReceiver   *otlptrace.Receiver
	metricsReceiver *otlpmetrics.Receiver

	traceConsumer   consumer.OTLPTraceConsumer
...
func New(addr string, tc consumer.OTLPTraceConsumer, mc consumer.MetricsConsumer, opts ...Option) (*Receiver, error) {
...
func (ocr *Receiver) registerTraceConsumer() error {
	...
	ocr.startTraceReceiverOnce.Do(func() {
		ocr.traceReceiver, err = otlptrace.New(ocr.traceConsumer, ocr.traceReceiverOpts...)
		if err == nil {
			srv := ocr.grpcServer()
			otlpagenttrace.RegisterTraceServiceServer(srv, ocr.traceReceiver)
		}
	})
...
func (ocr *Receiver) registerMetricsConsumer() error {
	...
	ocr.startMetricsReceiverOnce.Do(func() {
		ocr.metricsReceiver, err = otlpmetrics.New(ocr.metricsConsumer, ocr.metricsReceiverOpts...)
		if err == nil {
			srv := ocr.grpcServer()
			otlpagentmetric.RegisterMetricsServiceServer(srv, ocr.metricsReceiver)
		}
	})
	return err
}
