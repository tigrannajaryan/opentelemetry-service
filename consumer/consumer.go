...
// OTLPTraceConsumer is an interface that receives consumerdata.OTLPTrace, process it as needed, and
// sends it to the next processing node if any or to the destination.
type OTLPTraceConsumer interface {
	// ConsumeOTLPTrace receives consumerdata.OTLPTrace for processing.
	ConsumeOTLPTrace(ctx context.Context, td consumerdata.OTLPTrace) error
}
